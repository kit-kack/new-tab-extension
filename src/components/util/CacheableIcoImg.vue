<template>
  ...
  <template v-else>
    <img width="16"
         :src="finalIcoUrl"
         @error="useDefaultIcon = true; isloadedErrorAsCors = false"
         @load="dealWithCors"
...
const finalIcoUrl = ref(props.url)
let isloadedErrorAsCors = false;
function resolvedUrl(){
  const hostname = getHostname(props.url)
  if(hostname === null){
    return;
  }
  let ico = localStorage.getItem('ico-'+hostname)
  if(ico === null){
    fetchIco(hostname)
    return;
  }
  finalIcoUrl.value = ico;
  useDefaultIcon.value = false;
}
function fetchIco(hostname){
  const  finalUrl = icos.value.apis[icos.value.index].url.replace("%u",hostname);
  if(icos.value.apis[icos.value.index].nonCacheable){
    finalIcoUrl.value = finalUrl;
    useDefaultIcon.value = false;
    return;
  }
  // 获取ICO资源
  ...
  img.onload = ()=>{
    setTimeout(()=>{
      try{
        ...
        localStorage.setItem('ico-'+hostname,base64)
        console.log('cached ico '+hostname)
        finalIcoUrl.value =  base64;
        useDefaultIcon.value = false;
      }catch (_){
  ...
  img.onerror = ()=>{
    isloadedErrorAsCors = true;
    console.log("fetched [ "+img.src+" ] error, maybe as cors or network problem");
    finalIcoUrl.value = img.src;
    useDefaultIcon.value = false;
  }
}
function dealWithCors(){
  if(isloadedErrorAsCors){
    if(!icos.value.apis[icos.value.index].nonCacheable){
      icos.value.apis[icos.value.index].nonCacheable = true;
      restoreIcos();
    }
  }
}
